...
const optionsHandlerForIgnoreAndRemove = (selector, { ignore, remove } = {}) => {
...
const isolateInsideOfContainer = (containerSelectors, options) => {
  const whereNotExcept =
    typeof (options == null ? void 0 : options.except) === 'string' && options.except
  ...
  const prependWithCustomSelectors = (ruleSelector) =>
...
const isolateOutsideOfContainer = (containerSelectors, options) => {
  ...
  const insideOfContainerLogic =
    typeof (options == null ? void 0 : options.plus) === 'string' && options.plus
  ...
  return ({ ruleSelector }) => {
    ...
    return [
      ...
      insideOfContainerLogic == null
        ? void 0
        : insideOfContainerLogic({
            ruleSelector,
          }),
    ]
      .filter(Boolean)
...
const isolateForComponents = (componentSelectors, options) => {
  ...
  return ({ ruleSelector }) => {
    ...
    return (_optionsHandlerForIgn = optionsHandlerForIgnoreAndRemove(ruleSelector, options)) != null
      ? _optionsHandlerForIgn
...
const { withOptions } = TailwindPlugin__default['default'];
/**
    ...
    ({ addBase, corePlugins }) => {
      ...
      if (typeof modifyPreflightStyles === 'function') {
        ...
        modifyStylesHook = ({ selectorSet, property, value }) => {
          ...
          return matchingEntry == null || (_matchingEntry$ = matchingEntry[1]) == null
            ? void 0
            : _matchingEntry$[property];
        };
      }
      baseCssStyles.walkRules((rule) => {
        if (propsFilter || modifyPreflightStyles) {
          ...
          rule.nodes =
            (_rule$nodes = rule.nodes) == null
              ? void 0
              : _rule$nodes.map((node) => {
      ...
      addBase(
        baseCssStyles.nodes.filter((node, i, all) => {
          const next = all[i + 1];
          return node instanceof postcss__default['default'].Rule
            ? node.nodes.length > 0 && node.selector
            : node instanceof postcss__default['default'].Comment
              ? next instanceof postcss__default['default'].Rule &&
                next.selector &&
                next.nodes.length > 0
              : true;
        }),
